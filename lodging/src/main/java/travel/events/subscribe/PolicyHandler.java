...
public class PolicyHandler {
    ...
    @StreamListener(value = KafkaProcessor.INPUT, condition = "headers['type']=='LodgingReservationRequested'")
    ...
    @Retryable(value = RollbackException.class, maxAttempts = 3, backoff = @Backoff(delay = 1000))
    @StreamListener(value = KafkaProcessor.INPUT, condition = "headers['type']=='LodgingReservationCancelled' or headers['type']=='LodgingReservationFailed' or headers['type']=='LodgingReservationRefunded' or headers['type']=='LodgingReservationCancelRequested'")
    public void wheneverLodgingReservationCancelRequest_IncreaseRoomCapacity(
    ...
    @Recover
    public void recover(RollbackException e, Object RoomInfo) {
        logger.error("\n객실 수 수정에 실패했습니다.\n");
    }
}
// >>> Clean Arch / Inbound Adaptor
